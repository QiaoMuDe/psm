...
| 特性 | 说明 |
|:-----|:-----|
| � **极简搜索首页** | 百度式居中搜索框，300ms 防抖，支持键盘导航，快捷标签跳转 |
| �📝 **Prompt 管理** | 卡片/列表视图、分类筛选、标签搜索、JSON 导入导出、模板变量、置顶、批量操作 |
| 🧩 **Skill 管理** | 文件系统存储 + SKILL.md 自动解析，ZIP 双格式导入导出，编辑重命名同步目录 |
| 🤖 **AI 操作按钮** | 统一下拉菜单（生成名称/描述 + 优化），流式替换，还原机制，加载遮罩 |
| 🌐 **AI 翻译** | 左右分栏布局、流式翻译、6 种常用语言、语言交换、历史记录 |
| 🎨 **8 种主题** | light/dark/midnight/ocean/rose/lavender/purple/rosewood + auto 跟随系统 |
| 💾 **完整备份** | ZIP 格式全量备份，一键恢复，数据目录管理，孤立数据清理 |
| ⌨️ **快捷键** | 全局快捷键 + 模块快捷键 + Ctrl/Cmd+F 搜索聚焦 |
| 🖥️ **跨平台** | 基于 Wails v2，单文件部署，零外部依赖 |
...
构建完成后，可执行文件位于 `build/bin/psm.exe`。
