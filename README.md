...
| 特性 | 说明 |
|:-----|:-----|
| 📝 **Prompt 管理** | 增删改查、分类筛选、标签搜索、JSON 导入导出、模板变量、置顶 |
| 🧩 **Skill 管理** | 文件系统存储 + SKILL.md 自动解析，ZIP 双格式导入导出 |
| 🎨 **7 种主题** | light/dark/midnight/ocean/rose/lavender/auto，支持跟随系统 |
| 💾 **完整备份** | ZIP 格式全量备份，一键恢复，同名自动跳过 |
| ⌨️ **快捷键** | 全局快捷键 + 模块快捷键，操作更高效 |
