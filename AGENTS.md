...
│   ├── service/
│   │   ├── settings.go              # 设置服务：Get/Update/BatchUpdate/GetAppHome/GetSkillStoragePath
...
| 模块 | 核心功能 | 文件 | 核心输入/输出 |
|------|----------|------|---------------|
| 设置服务 | 系统参数 CRUD、程序家目录管理（读取/迁移）、重置默认设置 | `internal/service/settings.go` | 输入: key/value → 输出: map/string |
...
9. **纯 Go SQLite**: modernc.org/sqlite 无需 CGO，单 exe 部署
...
1. **数据库路径**: `~/.psm/data.db`（SQLite，WAL 模式），永远固定不迁移
2. **程序家目录**: `app_home` 设置项（默认 `~/.psm/`），Skill（`app_home/skills/`）和备份（`app_home/backup/`）跟随此目录
3. **一键备份**: 固定路径 `app_home/backup/psm-backup.zip`，每次覆盖，无文件对话框
4. **SKILL.md 规范**: YAML frontmatter 格式，`---\nname: xxx\ndescription: xxx\n---`
5. **Skill 导出双格式**: 单个 Skill → 标准格式（`ZipDir`）；多个 Skill → PSM 格式（`.psm-skill-export` + 技能目录）
6. **Skill 导入双格式**: 有标识文件 → 批量导入（`ImportSkillFromExportZip`）；无标识文件 → 单个导入（`ImportSkill`），均通过 `ImportSkillAuto` 统一入口
7. **SKILL.md 编辑同步**: `UpdateSkillFrontmatter` 仅替换 frontmatter 区域，不覆盖正文；文件不存在时自动创建
8. **Skill 模型无 version 字段**: 已从数据库模型、SQL 查询、前端表单中完全移除
9. **前端 ID 处理**: HTML data-id 是字符串，api.js 中所有 id 参数用 `Number()` 转换
10. **Go 空切片**: 所有返回前端的切片必须初始化为 `[]Type{}`，否则 JSON 输出 `null`
11. **构建命令**: `wails dev`（开发）/ `wails build`（生产）/ `go run tools/seed/main.go`（测试数据：21 条 Prompt 含 6 条模板 + 13 个 Skill）
12. **设计系统**: Flat Design，5 级阴影系统，3 级动画效果，Space Grotesk 字体，SVG 图标
13. **CSS 三文件**: `variables.css`（159 行变量/主题定义）、`layout.css`（377 行布局）、`components.css`（1300+ 行组件样式）
14. **前端路由**: app.js loadScript 去重机制，防止 const 重复声明
15. **默认视图模式**: prompt_view_mode 和 skill_view_mode 默认值为 `card`（数据库 INSERT OR IGNORE，不覆盖已有设置）
16. **批量管理模式**: `batchMode` 状态 + `.batch-mode` CSS 类切换，checkbox 默认隐藏，批量操作栏 `order:1` 固定底部
17. **右键菜单**: `ContextMenu` 组件，`show(x, y, items)` API，自动边界检测，点击外部/滚动自动关闭
18. **仪表盘导航**: 统计卡片通过 `data-view` 属性 + `App.navigate()` 实现点击跳转
19. **ZIP 目录扁平化**: `FlattenIfNested` 修复 ZIP 解压后多余的目录层级嵌套问题
20. **Prompt 卡片数据**: `allPrompts` 数组缓存加载结果，右键菜单通过 ID 查找数据
21. **版本信息**: verman 库（`gitee.com/MM-Q/verman`）构建时注入，前端 `loadVersion()` 展示，未注入时显示 "dev"
22. **6 种主题**: light（默认）、dark、midnight（午夜蓝）、ocean（海洋蓝）、rose（玫瑰红）、lavender（薰衣草紫）+ auto（跟随系统），均在 variables.css 定义
23. **设置页保存按钮**: `.form-actions` 使用 `justify-content: flex-end` 右对齐
24. **golangci-lint**: errcheck（`defer xxx.Close()` → 闭包包装，非 defer → `_ =` 前缀）+ staticcheck（错误信息小写）
25. **全局拖拽导入**: Wails `EnableFileDrop: true` + `OnFileDrop(callback, false)` 关键：`useDropTarget` 必须为 `false`，否则回调不会触发
26. **拖拽导入已存在处理**: `ImportSkillAuto` 和 `BatchImportSkills` 中检查错误消息包含"已存在"则计入 `Skipped` 而非 `Failed`
27. **快捷键管理**: `ShortcutManager` 对象，`registerView(name, shortcuts)` 注册模块快捷键，Ctrl+A 使用 `e.preventDefault()` 阻止默认全选行为
28. **Prompt 字符计数**: `input` 事件监听 textarea，`.char-count` / `.char-count-inline` 显示字符数
29. **数据统计 API**: `GetDataStats` 返回 `{prompt_count, skill_count, db_size}`，`GetOrphanSkills` 检测孤立 Skill
30. **搜索防抖**: 100ms `setTimeout/clearTimeout` 模式，`_searchTimer` 属性存储定时器 ID
31. **Handler 嵌入模式**: app.go 仅 ~73 行，4 个 Handler 文件通过结构体嵌入提供方法，Wails 反射可扫描嵌入方法
32. **数据库索引**: `idx_prompts_category`、`idx_prompts_updated_at`、`idx_skills_updated_at`
33. **置顶功能**: `is_pinned` 字段（DEFAULT 0），排序 `is_pinned DESC, updated_at DESC`，TogglePin 方法用 CASE WHEN 切换
34. **搜索高亮**: `highlightText(text, keyword)` 函数，先转义 HTML 再正则替换，`<mark>` 标签红色加粗显示
35. **标签点击筛选**: 点击标签填入搜索框，复用现有搜索逻辑（tags LIKE 匹配），无需独立筛选状态
36. **全局字体偏移**: CSS 变量 `--font-size-offset: 0px`，所有 `font-size` 使用 `calc(XXpx + var(--font-size-offset))`
37. **数据库字体设置**: `font_size_offset` 设置项，存储用户选择的字体大小偏移值
38. **关于弹窗**: `app.js showAboutDialog()` 显示版本号、项目地址、快捷键帮助
39. **仪表盘全局搜索**: 300ms 防抖，调用 `API.searchPrompts` 和 `API.searchSkills`，候选下拉菜单最多 8 条
40. **仪表盘置顶内容**: `API.getPinnedPrompts(3)` + `API.getPinnedSkills(3)`，各最多 3 个
41. **跳转闪烁**: `App.navigate(viewName, highlightId)` 传递 ID，`highlightItem(highlightId)` 执行高亮，3 秒动画
42. **悬停复制**: `hoveredPromptId` 追踪悬停 ID，Ctrl+C 快捷键复制内容到剪贴板
43. **模板变量格式**: 新建/编辑弹窗中显示 `{{变量名}}` 或 `{{变量名|默认值}}`，支持中文变量名，默认值留空时自动填充
44. **UI 阴影变量**: `--shadow-sm/md/lg/xl/2xl`，暗色主题阴影使用 `rgba(0,0,0,0.3)`
45. **动画过渡变量**: `--transition-fast: 150ms`、`--transition: 250ms`、`--transition-slow: 400ms`，Material Design 缓动曲线
46. **跳转闪烁动画**: `@keyframes highlight-flash`，红色边框+背景闪烁，3 秒后移除类
47. **全局字体族变量**: CSS 变量 `--font-family`，默认 `'Space Grotesk', -apple-system, ...`
48. **数据库字体族设置**: `font_family` 设置项，存储用户选择的字体族名称
49. **系统字体获取**: `internal/utils/font.go` 使用 Windows API `EnumFontFamiliesW` 获取系统字体列表
50. **字体下拉滚动**: 下拉框显示时 `scrollIntoView({ block: 'nearest' })` 自动滚动到选中项
51. **字体回退机制**: 如果用户选择的字体不存在，自动回退到默认字体
52. **搜索框键盘导航**: `_searchIndex` 状态 + `updateSearchHighlight` 方法，上下键移动高亮，回车选择
53. **数据重置 API**: `ResetAllData` 方法，清空 prompts 表 + 删除 skills 文件 + 清空 skills 表 + 重置 settings 表
54. **二次确认保护**: 数据重置操作需要两次确认，防止误操作
55. **工具栏不换行**: `.toolbar` 移除 `flex-wrap: wrap`，`.toolbar-right` 添加 `flex-wrap: nowrap`
56. **技能空状态提示**: 改为"点击'导入技能'添加第一条"，反映功能变更
57. **点击动画**: CSS `:active` 伪类实现按下瞬间缩放（`transform: scale(0.98)`），比 JS click 事件更可靠
58. **Skill 详情弹窗**: 简洁信息流布局，`skill-detail-compact` 类，分隔线区分区块，无创建/更新时间显示
59. **文件右键菜单**: `.skill-detail-file-item` 元素绑定 contextmenu 事件，ContextMenu 选择器需包含此类
60. **ContextMenu 选择器**: 全局 contextmenu 事件需允许 `.skill-detail-file-item`，否则菜单被立即隐藏
61. **后端文件操作**: `RevealInExplorer(path)` 用 explorer 打开目录，`OpenFile(path)` 用 cmd start 打开文件
62. **SkillFile FullPath**: 模型新增 `full_path` 字段，`ListSkillFiles` 返回完整路径供前端使用
