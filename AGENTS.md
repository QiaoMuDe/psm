...
> 版本: 2.1.0 | 更新日期: 2026-05-30 | 分析人: AI 架构师
...
│   ├── css/
│   │   ├── variables.css            # CSS 变量定义：颜色/间距/字体/阴影/圆角
│   │   ├── layout.css               # 布局样式：应用容器/侧边栏/主内容区/响应式/视图内容滚动
│   │   └── components.css           # 组件样式：卡片/按钮/表单/表格/标签/模态框/Toast/批量栏/右键菜单
│   └── js/
...
│           ├── skills.js            # Skill 管理：卡片/列表视图/搜索/CRUD/批量管理/右键菜单/ZIP 导入导出/文件浏览
...
| CSS 组织 | ✅ 三文件拆分：variables（变量）、layout（布局）、components（组件） |
...
→ 用户点击菜单项 → 执行对应 action（查看/复制/编辑/删除）
...
6. **CSS 三文件拆分**: variables.css（变量）、layout.css（布局）、components.css（组件），职责清晰
7. **纯 Go SQLite**: modernc.org/sqlite 无需 CGO，单 exe 部署
8. **主题系统**: CSS 变量驱动的亮/暗主题，设置持久化到数据库
9. **卡片视图右键菜单**: 卡片/表格行右键弹出操作菜单（查看/复制/编辑/删除），替代固定按钮，界面更简洁
...
12. **CSS 三文件**: `variables.css`（87 行变量定义）、`layout.css`（377 行布局）、`components.css`（960+ 行组件样式）
